...
fn a_estrategia_do_so_resolve_para_a_plataforma_corrente() {
    ...
    assert!(
        !s.ollama_install_steps().is_empty(),
        "todo SO precisa de um plano de instalacao"
    );
    assert!(
        s.data_dir().is_absolute(),
        "o diretorio de dados precisa ser absoluto"
    );
}
...
fn a_leitura_de_memoria_e_coerente() {
    ...
    assert!(
        ram.max_budget_bytes < ram.total_bytes,
        "o teto precisa deixar folga para o SO"
    );
    assert!(
...
fn o_catalogo_separa_o_que_a_maquina_aguenta() {
    ...
    for e in &entradas {
        assert_eq!(
            ...
            "{} foi classificado errado",
            e.spec.tag
        );
        assert!(
            !e.reason.is_empty(),
            "toda entrada precisa explicar o proprio veredito"
        );
    }
    ...
    let kimi = entradas
        .iter()
        .find(|e| e.spec.tag.starts_with("kimi"))
        .expect("kimi no catalogo");
    assert!(
        kimi.spec.focus,
        "kimi foi pedido explicitamente e precisa aparecer"
    );
    assert!(
        !kimi.supported,
        "1T de parametros nao cabe em hardware de mesa"
    );
}
...
fn falta_de_memoria_rebaixa_o_cargo_com_aviso() {
    ...
    assert!(
        ...
        "escolheu algo que nao cabe"
    );
    assert!(
        aviso.is_some(),
        "o rebaixamento precisa ser registrado para o usuario ver"
    );
...
fn o_organograma_impede_atalho_entre_cargos() {
    ...
    assert!(
        !Role::DiretorGeral.can_send_to(Role::Criador),
        "o diretor nao fala direto com o executor"
    );
    assert!(Role::GerenteSetor.can_send_to(Role::Criador));
    assert!(Role::Criador.can_send_to(Role::Auditor));
    assert!(
        !Role::Criador.can_send_to(Role::GerenteSetor),
        "o criador nao responde ao gerente por conta propria"
    );
    assert!(
        Role::Auditor.can_send_to(Role::GerenteSetor),
        "a decisao final e conjunta com quem manda"
    );
}
...
fn a_escolha_muda_com_o_hardware() {
    ...
    assert!(
        !com_gpu.moe,
        "com GPU esperava o denso mais capaz, veio {}",
        com_gpu.label
    );
...
fn o_perfil_desta_maquina_e_coerente() {
    ...
    if p.mode == ComputeMode::Cpu {
        ...
        assert!(
            p.prefers_moe,
            "sem acelerador o sistema tem de preferir MoE"
        );
    }
    ...
    println!(
        ...
        p.accelerators
            ...
            .map(|a| format!(
                "{} ({}, {})",
                a.name,
                hardware::human(a.vram_total_bytes),
